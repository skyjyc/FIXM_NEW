...
public class AircraftDAO extends BaseHibernateDAO {
	...
	public Aircraft findById(java.lang.String id) {
		...
		try {
			Aircraft instance = (Aircraft) getSession().get("com.fixmnew.bean.Aircraft", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Aircraft instance) {
		...
		try {
			List results = getSession().createCriteria("com.fixmnew.bean.Aircraft").add(Example.create(instance))
					.list();
